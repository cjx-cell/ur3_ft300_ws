...
**本轮唯一主线：验证四物理专家提供的条件是否真正改善完整动作生成及实际执行。不是继续整理 GitHub，也不是直接启动下一轮训练。**
...
首页保留采集数据可视化与 PAP-MoE 模型推理两个 GitHub 视频附件播放器。用户选定推理源为 `artifacts/gazebo_pap_moe_workspace50_20260908_195140_ep0031_seed2/pap_moe_pure_rollout.mp4`。原源为 mp4v，公开版已转为浏览器兼容 H.264/yuv420p，保留309帧/30.9秒；不能再直接上传 mp4v 或用 GIF 冒充视频播放器。详情在 `docs/assets/demo_manifest.json`，原实验录像未修改。
