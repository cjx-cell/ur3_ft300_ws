...
更新：2026-09-12，Stage 6 vNext 正式训练 12/12 完成。**本文是唯一当前状态入口；有日期的实验报告只描述当时合同，不作为默认启动命令。**

> **最新断点（覆盖下文较早的“没有训练”状态描述）：** 191点汇总、正常前缀复现偏差审计、6次native
> 标定、18次随机区组pilot、逐专家敏感性、Stage 5性质/完整Flow回放、Stage 6决策包、隔离vNext集成均已
> 完成。四专家完整条件收益证据仍为0/4。隔离分支冻结提交为
> `a3ddb6e5e1d7a45562105c0ef03942118fe30f82`；完整回归162项与overlay 53/53通过。用户其后只批准并已
> 完成 A→B→C→D、seed1000、每臂2步的短训门禁：4/4完成有限更新、保存和strict reload，一次性
> conditioner reset未在重载时再次清空。正式4臂×3 seed×10,000步已于2026-09-12 07:35完成：
> 12/12退出码0，每项200条训练记录的loss/梯度有限，5,000/10,000检查点齐全，one-shot reset均正确消费。
> A/seed1000曾因队列校验器不识别`step:10K`缩写被假报失败；修正后只续跑余下11项，未重跑或覆盖A。
> 终态见 `artifacts/deployment_validation_20260911_stage6_vnext_formal_12run/status.json`。这只证明四种结构在
> 冻结合同下可完整训练；尚未完成统一开环/闭环性能评估，不能按最终loss声明架构优劣。新数据、Gazebo、
> 机器人执行和阈值变化仍未授权。短训证据见
> `artifacts/deployment_validation_20260911_stage6_vnext_smoke_seed1000_2step/smoke_report.md`。

## 0. 新对话从这里开始

**本轮唯一主线：验证四物理专家提供的条件是否真正改善完整动作生成及实际执行。**

- 9月9日的 `core_information` 已完成四模型离线诊断，实际分支曾因起点复现不合格停止；后续阶段的最新进展与当前训练状态以本节顶部“最新断点”和对应状态文件为准。
