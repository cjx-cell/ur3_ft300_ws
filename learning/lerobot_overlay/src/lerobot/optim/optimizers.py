...
def _patch_bnb_state_dict(opt: torch.optim.Optimizer) -> torch.optim.Optimizer:
    """Monkey-patch bnb AdamW8bit state_dict/load_state_dict for safetensors compat.
    bnb stores optimizer 'step' as Python int; lerobot checkpointing uses
    safetensors which only supports float32/float64.  This patches the methods
    to cast step<->float on save/load.
    """
    import numpy as np

    orig_lsd = opt.load_state_dict

    def patched_load_state_dict(sd):
        for p_group, state in zip(opt.param_groups, sd["param_groups"]):
            for pid in p_group["params"]:
                if pid not in sd["state"]:
                    continue
                s = sd["state"][pid]
                if "step" in s and isinstance(s["step"], (np.floating, float)):
                    s["step"] = int(s["step"])
        orig_lsd(sd)

    opt.load_state_dict = patched_load_state_dict

    orig_ssd = opt.state_dict

    def patched_state_dict():
        sd = orig_ssd()
        for s in sd["state"].values():
            if "step" in s and isinstance(s["step"], int):
                s["step"] = float(s["step"])
        return sd

    opt.state_dict = patched_state_dict
    return opt


@OptimizerConfig.register_subclass("adamw8bit")
@dataclass
class AdamW8bitConfig(AdamWConfig):
    """AdamW with 8-bit quantized states via bitsandbytes."""
    ...
    def build(self, params: OptimizerParams) -> torch.optim.Optimizer:
        ...
        return _patch_bnb_state_dict(opt)
