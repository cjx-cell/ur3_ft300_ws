...
from .pap_moe.configuration_pap_moe import PAPMoEConfig
...
def get_policy_class(name: str) -> type[PreTrainedPolicy]:
    ...
    if name == "tdmpc":
        ...
        from .pap_moe.modeling_pap_moe import PAPMoEPolicy
...
def make_policy_config(policy_type: str, **kwargs) -> PreTrainedConfig:
    ...
    elif policy_type == "pap_moe":
        return PAPMoEConfig(**kwargs)
...
def make_pre_post_processors(
    ...
    **kwargs: Unpack[ProcessorConfigKwargs],
    ...
    """
    if isinstance(policy_cfg, PAPMoEConfig) and kwargs.get("dataset_stats") is not None:
        from .pap_moe.processor_pap_moe import make_pap_moe_pre_post_processors

        return make_pap_moe_pre_post_processors(
            config=policy_cfg,
            dataset_stats=kwargs["dataset_stats"],
        )

    if pretrained_path:
        # TODO(Steven): Temporary patch, implement correctly the processors for Gr00t
        if type(policy_cfg).__name__ == "GrootConfig":
        ...
        if isinstance(policy_cfg, PAPMoEConfig):
            from .pap_moe.processor_pap_moe import (
                PapMoePrepareStateTokenizerProcessorStep,
                make_pap_moe_pre_post_processors,
            )

            uses_pap_prompt = any(
                isinstance(step, PapMoePrepareStateTokenizerProcessorStep)
                for step in preprocessor.steps
            )
            if not uses_pap_prompt:
                dataset_stats = kwargs.get("dataset_stats")
                if dataset_stats is None:
                    raise ValueError(
                        "The pretrained processor is not PAP-MoE compatible and "
                        "dataset_stats were not provided to construct one."
                    )
                return make_pap_moe_pre_post_processors(
                    config=policy_cfg,
    ...
    elif isinstance(policy_cfg, PAPMoEConfig):
        from .pap_moe.processor_pap_moe import make_pap_moe_pre_post_processors

        processors = make_pap_moe_pre_post_processors(
            config=policy_cfg,
            dataset_stats=kwargs.get("dataset_stats"),
        )
...
def make_policy(
    ...
    # Auto-load force/stage normalization stats from dataset metadata.
    # SA-MOE policies need per-dimension force stats and stage denorm params
    # that are dataset-specific; this avoids hardcoded parameter mismatch.
    if hasattr(policy, 'load_force_stats') and ds_meta is not None:
        policy.load_force_stats(ds_meta.stats)
