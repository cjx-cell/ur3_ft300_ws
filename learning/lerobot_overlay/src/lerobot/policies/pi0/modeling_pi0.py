...
class PI0Policy(PreTrainedPolicy):
    ...
    def _fix_pytorch_state_dict_keys(
        ...
        for key, value in state_dict.items():
            ...
            # pi0_libero_base saved with older PaliGemma that had vision_model submodule
            new_key = new_key.replace(".vision_model.", ".")

            # Fix 2: Add "model." prefix to keys that lack it
            # (pi0_libero_base has mixed prefixes — some keys start with "model.",
            #  others like "action_in_proj.bias" don't)
    ...
    def _get_default_peft_targets(self) -> dict[str, any]:
        ...
        Targets all Gemma attention & MLP projections (VLM + action expert).
        Projections are fully fine-tuned (modules_to_save).
        """
        # Match all Gemma layers: self_attn Q/K/V/O + MLP gate/up/down
        target_modules = (
            r".*\.self_attn\.(q_proj|k_proj|v_proj|o_proj)$"
            r"|.*\.mlp\.(gate_proj|up_proj|down_proj)$"
        )
        # Projections are fully trained (not LoRA)
        modules_to_save = [
