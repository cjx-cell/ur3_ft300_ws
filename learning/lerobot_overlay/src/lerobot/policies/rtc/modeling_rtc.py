...
class RTCProcessor:
    ...
    def denoise_step(
        ...
        Args:
            ...
            action_mask (Tensor | None): Optional per-action-dimension RTC mask. This
                is useful for excluding discrete action dimensions such as a binary
                gripper while still guiding continuous arm joints.
        ...
        if action_mask is not None:
            action_mask = torch.as_tensor(action_mask, device=x_t.device, dtype=weights.dtype)
            if action_mask.ndim == 1:
                action_mask = action_mask.unsqueeze(0)
            if action_mask.ndim != 2 or action_mask.shape[0] not in (1, batch_size):
                raise ValueError(
                    "RTC action_mask must have shape [A] or [B, A], got "
                    f"{tuple(action_mask.shape)}"
                )
            if action_mask.shape[1] > action_dim:
                raise ValueError(
                    f"RTC action_mask has {action_mask.shape[1]} dimensions, expected at most {action_dim}"
                )
            if action_mask.shape[1] < action_dim:
                action_mask = torch.nn.functional.pad(
                    action_mask, (0, action_dim - action_mask.shape[1])
                )
            weights = weights * action_mask.unsqueeze(1)

        # ``x_t`` is intentionally detached here, so the existing RTC
        # approximation does not differentiate through the denoiser Jacobian.
        # Keep the expensive policy forward out of autograd as well; otherwise
        # trainable policy parameters retain a large, unused graph at inference.
        with torch.no_grad():
            v_t = original_denoise_step_partial(x_t)
