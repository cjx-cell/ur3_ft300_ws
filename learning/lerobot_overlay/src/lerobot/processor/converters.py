...
def _extract_complementary_data(batch: dict[str, Any]) -> dict[str, Any]:
    ...
    # Auxiliary supervision is not an observation and must never be normalized
    # or fed to an encoder. Preserve these optional PAP-MoE targets only in
    # complementary_data so the policy loss can consume them without leakage.
    auxiliary_supervision_keys = (
        "skill_progress_phase",
        "skill_progress",
        "transition_readiness",
        "skill_progress_valid",
        "skill_progress_confidence",
    )
    auxiliary_supervision = {
        key: batch[key] for key in auxiliary_supervision_keys if key in batch
    }
