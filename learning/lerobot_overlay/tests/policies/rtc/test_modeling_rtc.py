...
def test_denoise_step_action_mask_excludes_discrete_dimension(rtc_processor_debug_disabled):
    x_t = torch.ones(1, 20, 2)
    prev_chunk = torch.full((1, 20, 2), 0.1)
    ...
    assert not torch.allclose(result[..., 0], torch.full_like(result[..., 0], 0.5))
    assert torch.allclose(result[..., 1], torch.full_like(result[..., 1], 0.5))
