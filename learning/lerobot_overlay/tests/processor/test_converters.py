...
def test_batch_transition_roundtrip_preserves_auxiliary_supervision_without_observation_leakage():
    """PAP-MoE targets remain loss-only complementary data across conversion."""
    supervision = {
        "skill_progress_phase": torch.tensor([2, 4], dtype=torch.int64),
        "skill_progress": torch.tensor([0.25, 0.75]),
        "transition_readiness": torch.tensor([0.1, 0.9]),
        "skill_progress_valid": torch.tensor([True, False]),
        "skill_progress_confidence": torch.tensor([0.85, 0.70]),
    }
    batch = {
    ...
    assert set(observation) == {OBS_STATE}
    for key, value in supervision.items():
        assert key not in observation
        assert torch.equal(complementary_data[key], value)

    restored_batch = transition_to_batch(transition)
    for key, value in supervision.items():
        assert torch.equal(restored_batch[key], value)
