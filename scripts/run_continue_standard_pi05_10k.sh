#!/bin/bash
# Continue the corrected global-task Pi0.5 baseline from its 1k checkpoint.
# Optimizer/scheduler are intentionally restarted in a new output directory,
# while all learned policy weights are loaded from the validated checkpoint.
set -euo pipefail

WS_DIR="/home/ubuntu/ur3_ft300_ws"
PYTHON_ENV="/home/ubuntu/miniconda3/envs/pi0-env/bin/python"
SOURCE_CHECKPOINT="$WS_DIR/outputs/train/pi05_relative_h50_global_task_overfit_5ep/checkpoints/001000/pretrained_model"
DATASET_ROOT="$WS_DIR/pap_moe_framework/datasets/lerobot_overfit_5ep_relative_h50_global_task"
OUTDIR="$WS_DIR/outputs/train/pi05_relative_h50_global_task_continue_10k"
...
if [ ! -f "$DATASET_ROOT/GLOBAL_TASK_VIEW.json" ]; then
  echo "ERROR: validated global-task dataset view is missing: $DATASET_ROOT" >&2
  exit 2
fi
if [ -e "$OUTDIR" ]; then
...
echo "================================================================="
echo "  Pi0.5 Global-Task Continuation Training"
echo "  Source:   $SOURCE_CHECKPOINT"
echo "  Dataset:  $DATASET_ROOT"
echo "  Steps:    10000 additional updates (~3.14 dataset passes)"
echo "  Batch:    1"
echo "  Output:   $OUTDIR"
echo "================================================================="
