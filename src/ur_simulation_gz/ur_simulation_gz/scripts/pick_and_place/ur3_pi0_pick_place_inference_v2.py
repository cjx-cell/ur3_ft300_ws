...
class UR3Pi0Inference:
    def __init__(self, mode="cpu", hz=5, model_dir=None, ema_alpha=0.1):
        self.mode = mode
        self.hz = hz
    ...
    def _load(self):
        ...
        # 也可以通过 normalizer 自动检测: relative 模式下 arm 关节 action_mean ≈ 0
        # 注意: 只检查 arm 关节 (排除 gripper)，因为 gripper 可能被 relative_exclude_joints 排除
        arm_action_mean = np.abs(self.action_mean[:6]).max()  # 前6个是 arm 关节
        if not self.use_relative and arm_action_mean < 0.01:
            self.use_relative = True
            print("  自动检测到相对动作模式 (arm action_mean≈0)")
        if self.use_relative:
            print("  ✓ 相对动作模式: 推理输出 delta + state → 绝对关节角")
        elif arm_action_mean > 0.1:
            # action_mean 远离 0，可能是未修复的绝对动作统计量
            print("  ⚠ 检测到绝对动作统计量 (arm action_mean={:.4f} rad)".format(arm_action_mean))
            print("  ⚠ 如果训练时 use_relative_actions=true，请先运行 fix_relative_action_stats.py 修复统计量")
        else:
    ...
    def run(self):
        ...
        # 动作 EMA 平滑，减少抖动
        ema_action = None
        ema_alpha = getattr(self, 'ema_alpha', 0.1)  # 平滑系数 (0=不平滑, 1=完全用历史)
...
def main():
    ...
    parser.add_argument("--ema-alpha", type=float, default=0.1,
                        help="EMA 平滑系数 (0=不平滑, 1=完全历史, 默认 0.1)")
